...
func (s *BookService) GetDataService() ([]types.Booktypes, error) {
	...
	query := `
		SELECT  ID,Ten,IDloai,Soluong,Gia
		FROM Book		
	`
...
func (s *BookService) OderDataService(requestParams *request.Oder) (types.Odertypes, error) {
	...
	insertOrderQuery := `
	INSERT INTO Oder
		(Ngaydathang, IDbook, IDkhachhang, Soluong, Gia)
	VALUES
		(?, ?, ?, ?, ?);
	`
...
func (s *BookService) GetOrderStatistics() ([]types.OrderStat, error) {
	var orders []types.OrderStat
	...
	query := `
		SELECT Ngaydathang AS order_date, 
       SUM(Soluong * Gia) AS total_revenue,
       SUM(Soluong) AS total_books_sold
		FROM Oder
